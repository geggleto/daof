 ...
 */
export function resolvePathUnderBase(filePath: string, baseDir: string): string {
  ...
  if (rel.startsWith("..")) {
    throw new Error(
      `Path escapes allowed base: ${filePath} (resolved: ${resolved}, base: ${baseReal})`
    );
  }
  return toCheck;
}
 ...
 */
export function sanitizeCapabilityIdForPath(id: string): string {
  if (typeof id !== "string" || id.length === 0) {
    throw new Error("Capability id must be a non-empty string");
  }
  if (!SAFE_CAPABILITY_ID_REGEX.test(id)) {
    throw new Error(
      `Capability id contains disallowed characters (use only [a-zA-Z0-9_-]): ${id}`
    );
  }
  return id;
}
