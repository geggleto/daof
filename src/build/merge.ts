...
export function stripMarkdownLines(text: string): string {
  ...
  for (const line of lines) {
    ...
    if (/^\d+\.\s+.+$/.test(t)) continue; // numbered list lines
    out.push(line);
  }
  return out.join("\n").trim();
}
...
export function looksLikeYamlContent(text: string): boolean {
  const t = text.trim();
 ...
 */
export function findMentionedYamlPath(text: string): string | undefined {
 ...
 */
export function extractYamlFromMarkdown(text: string): string {
  const trimmed = text.trim();
  if (!trimmed) return trimmed;
  try {
    const tokens = lexer(trimmed);
    for (const token of tokens) {
      if (token.type === "code" && "text" in token && typeof (token as { text: string; lang?: string }).text === "string") {
        const t = token as { text: string; lang?: string };
        const lang = (t.lang ?? "").toLowerCase();
        if (lang === "yaml" || lang === "yml" || !lang) return t.text.trim();
      }
    }
  } catch {
 ...
 */
export function extractCodeBlock(text: string, preferLang?: string): string | undefined {
  const trimmed = text.trim();
  ...
  try {
    ...
    for (const token of tokens) {
      if (token.type === "code" && "text" in token && typeof (token as { text: string; lang?: string }).text === "string") {
        ...
        if (preferLang) {
          ...
          if (lang === want || (want === "typescript" && lang === "ts")) return t.text.trim();
        } else {
          return t.text.trim();
        }
      }
    ...
    if (preferLang) {
      for (const token of tokens) {
        if (token.type === "code" && "text" in token && typeof (token as { text: string }).text === "string")
          return (token as { text: string }).text.trim();
      }
    }
  } catch {
    /* ignore */
...
export function extractGenerated(parsed: ParsedYaml): ExtractedGenerated {
  if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) {
    return { capabilities: {}, agents: {}, workflows: {} };
  }
  const obj = parsed as Record<string, unknown>;
  const capabilities =
    obj.capabilities && typeof obj.capabilities === "object" && !Array.isArray(obj.capabilities)
  ...
  const agents =
    obj.agents && typeof obj.agents === "object" && !Array.isArray(obj.agents)
      ...
      : {};
  const workflows =
    obj.workflows && typeof obj.workflows === "object" && !Array.isArray(obj.workflows)
...
export function mergeIntoConfig(
  config: OrgConfig,
  generated: ExtractedGenerated
): OrgConfig {
  return {
    ...config,
    capabilities: { ...config.capabilities, ...generated.capabilities } as OrgConfig["capabilities"],
    agents: { ...config.agents, ...generated.agents } as OrgConfig["agents"],
    workflows: { ...config.workflows, ...generated.workflows } as OrgConfig["workflows"],
  };
}
