...
import { extractYamlFromMarkdown, looksLikeYamlContent, extractGenerated, mergeIntoConfig, } from "./merge.js";
...
import { runPlannerAgent, runGeneratorAgent, runMergeAndWriteAgent, runVerifierAgent, } from "./agents.js";
...
const BUILD_AGENT_IDS = ["planner", "generator", "builder", "verifier"];
function askProceed() {
...
function hasBuildAgents(runtime) {
 ...
 */
export async function runPlanner(description, options) {
    ...
    if (!service) {
        throw new Error(`Planner requires a provider (e.g. ${providerId}) with API key. Set ${providerId === "cursor" ? "CURSOR_API_KEY" : "provider API key"} in the environment.`);
    }
    ...
    if (!plannerResult || ("ok" in plannerResult && plannerResult.ok === false)) {
        throw new Error("ok" in plannerResult && plannerResult.ok === false ? plannerResult.error : "Planner failed");
    }
    const prd = ("text" in plannerResult ? plannerResult.text : "").trim();
    if (!prd)
        throw new Error("Planner returned empty PRD.");
    if (verbose >= 1)
        console.error("[build] Planner done.");
    return prd;
}
/**
 ...
 */
export async function runPlannerRevise(prd, userFeedback, options) {
    const { providerId, verbose = 0 } = options;
    const service = getProviderService(providerId, getProviderApiKey(providerId));
    if (!service) {
        throw new Error(`Planner revise requires a provider (e.g. ${providerId}) with API key. Set ${providerId === "cursor" ? "CURSOR_API_KEY" : "provider API key"} in the environment.`);
    }
    ...
    if (!result || ("ok" in result && result.ok === false)) {
        throw new Error("ok" in result && result.ok === false ? result.error : "Planner revise failed");
    }
    const revised = ("text" in result ? result.text : "").trim();
    if (!revised)
        throw new Error("Planner returned empty PRD.");
    if (verbose >= 1)
        console.error("[build] Planner revise done.");
    return revised;
}
/**
 ...
 */
export async function runBuild(description, options) {
    ...
    if (!useAgents) {
        ...
        if (!service) {
            return {
                success: false,
                error: new Error(`daof build requires a provider (e.g. ${providerId}) with API key. Set ${providerId === "cursor" ? "CURSOR_API_KEY" : "provider API key"} in the environment.`),
            };
        }
    }
    ...
    if (options.initialPrd) {
        ...
        if (verbose >= 1)
            console.error("[build] Using provided initial PRD.");
    }
    else {
        const plannerSpinner = ora("Planning…").start();
        if (verbose >= 1)
            console.error("[build] Running planner...");
        try {
    ...
    for (let attempt = 0; attempt < MAX_VERIFIER_RETRIES; attempt++) {
        ...
        try {
            if (useAgents) {
                const genSpinner = ora(attempt > 0 ? `Retrying… (attempt ${attempt + 1}/${MAX_VERIFIER_RETRIES})` : `Generating… (attempt ${attempt + 1}/${MAX_VERIFIER_RETRIES})`).start();
                if (verbose >= 1)
                    console.error(`[build] Running generator (attempt ${attempt + 1}/${MAX_VERIFIER_RETRIES})...`);
                let yaml;
                try {
                    yaml = await runGeneratorAgent(runtime, description, prd, existingCapabilityIds);
                    genSpinner.succeed("Generator done.");
                    if (verbose >= 1)
                        console.error("[build] Generator done.");
                }
                catch (e) {
                    genSpinner.fail("Generator failed.");
                    throw e;
                }
                let generatedForMerge;
                try {
                ...
                if (verbose >= 1)
                    console.error("[build] Checking similarity...");
                try {
                    ...
                    if (verbose >= 1)
                        console.error("[build] Similarity check done.");
                ...
                try {
                    ...
                    if (verbose >= 1)
                        console.error(`[build] Merge done. ${mergeOut.summary}`);
                }
                catch (e) {
                ...
                if (verbose >= 1)
                    console.error("[build] Running verifier...");
                let pass;
                try {
                    ...
                    if (pass) {
                        ...
                        if (verbose >= 1)
                            console.error("[build] Verifier pass.");
                    }
                    else {
                        verSpinner.fail("Verifier fail.");
                        if (verbose >= 1)
                            console.error("[build] Verifier fail.");
                    }
                }
                catch (e) {
                ...
                if (pass) {
                    if (didCodegen)
                        console.log("Run `npm run build` to compile and include the new capabilities.");
                ...
                try {
                    ...
                    lastError = !looksLikeYamlContent(stripped)
                        ? new Error("Generator did not return valid YAML in the response (it may have written to a file or returned only a summary). The generator must output the full YAML in the response, e.g. inside a ```yaml code block. Original parse error: " +
                            parseErr.message)
                        : parseErr;
                    if (verbose >= 2)
                        console.error("[build] YAML parse error:", parseErr.message);
                    continue;
                }
                const generated = extractGenerated(parsed);
                if (Object.keys(generated.capabilities).length === 0 &&
                ...
                const addedCount = Object.keys(generated.capabilities).length +
                    Object.keys(generated.agents).length +
                    Object.keys(generated.workflows).length;
                const summary = `Capabilities: ${Object.keys(generated.capabilities).join(", ") || "none"}. ` +
                ...
                if (verbose >= 1)
                    console.error(`[build] Merge done. ${summary}`);
                if (!noCodegen) {
                    const codegenResult = bundle
