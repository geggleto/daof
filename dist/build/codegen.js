...
export async function runCodegenForOne(providerId, id, def, verbose) {
    const apiKey = getProviderApiKey(providerId);
    const service = getProviderService(providerId, apiKey);
    if (!service)
        throw new Error("Codegen requires provider with API key.");
    ...
    for (let r = 0; r < CODEGEN_RETRIES; r++) {
        try {
            ...
            if (verbose >= 2)
                console.error("[build] Codegen attempt " + (r + 1) + "/" + CODEGEN_RETRIES + " for " + id + ": " + lastErr.message);
        }
    }
    throw new Error("Capability codegen failed for " + id + " after " + CODEGEN_RETRIES + " attempts: " + (lastErr?.message ?? "Unknown error") + ". Check description/config and try again.");
}
/** Derive bundled factory name from capability id (snake_case -> createXxxInstance). */
export function capabilityIdToFactoryName(id) {
    const pascal = id
        .split("_")
 ...
 */
export function normalizeBundledExport(code, factoryName) {
    const match = code.match(/export\s+default\s+function\s+(\w+)/);
    if (!match)
        return { code, usedNamedExport: false };
 ...
 */
export function patchBundledIndex(bundledDir, additions, verbose) {
    ...
    const toAdd = additions.filter(({ id }) => {
        ...
        if (already && verbose >= 1)
            console.error("[build] Bundled registry already has " + id + ", skipping.");
        return !already;
    });
    if (toAdd.length === 0)
        return;
    ...
    if (registryClose === -1)
        throw new Error("bundled index.ts: could not find registry closing };");
 ...
 */
export async function runCodegenPhase(providerId, orgFilePath, newCapabilityIds, codegenDir, verbose) {
 ...
 */
export async function runCodegenPhaseForBundle(providerId, orgFilePath, newCapabilityIds, verbose) {
    const cwd = process.cwd();
    const bundledDir = join(cwd, "src", "capabilities", "bundled");
    const indexPath = join(bundledDir, BUNDLED_INDEX_FILENAME);
    if (!existsSync(indexPath)) {
        throw new Error("Cannot use --bundle: run from the DAOF repo root so that src/capabilities/bundled exists.");
    }
    const raw = loadYaml(orgFilePath);
    const config = validate(raw);
    const additions = [];
    ...
    for (const id of newCapabilityIds) {
        ...
        if (verbose >= 1)
            console.error("[build] Codegen " + id + " (bundle)...");
        try {
            const code = await runCodegenForOne(providerId, id, def, verbose);
            ...
            if (!usedNamedExport) {
                throw new Error(`Generated code for ${id} has no default-exported function; bundled capabilities must use "export default function ..." so the build can normalize to a named export.`);
            }
            ...
            if (verbose >= 1)
                console.error("[build] Codegen " + id + " (bundle) done.");
        }
        catch (e) {
    ...
    if (additions.length > 0)
        patchBundledIndex(bundledDir, additions, verbose);
    for (const { id } of additions) {
        const cap = config.capabilities[id];
        if (cap)
            delete cap.source;
    }
    if (didCodegen)
